...
export default function ProductDetailActions({ product }: ProductDetailActionsProps) {
  ...
  const waMessage = encodeURIComponent(
    `Hello Al Mobeen Enterprise, I am inquiring about bulk availability and pricing for: ${product.displayName}.`
  );
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-[var(--color-border)]">
      <button
        ...
        className={`px-6 py-3 rounded-full text-sm font-semibold flex items-center gap-2 transition-all ${
          isAdded
      ...
      >
        {isAdded ? (
          <>
            ...
            <span>{t("products.addedToQuote")}</span>
          </>
        ) : (
          <>
            ...
            <span>{t("products.addToQuote")}</span>
          </>
        )}
      </button>
      ...
      >
        ...
        <span>{t("products.inquireWhatsApp")}</span>
      </a>
    </div>
  );
}
